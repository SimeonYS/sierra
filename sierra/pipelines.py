...
class SierraPipeline:

    # Database setup
    conn = sqlite3.connect('sierra.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sierra`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sierra WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sierra (date, title, link, content)"
